package factor.labs.indiancalendar.CalendarUI.CalendarHeaderList;
...
public class HeaderListView extends RelativeLayout {
    ...
    private DayOnHeaderClickListener mClickCallback;
    ...
    public int setSelection(int section, int row){
        int offset = mAdapter.getSelectionOffset(section, row);
        mListView.setSelection(offset+row);
        return offset;
    }

    public void hideHeader(){ if(mHeader != null) mHeader.setVisibility(View.GONE); }
    public void showHeader(){ if(mHeader != null) mHeader.setVisibility(View.VISIBLE); }
    ...
    public void setHeaderListener(DayOnHeaderClickListener oClickCallback){
        mClickCallback = oClickCallback;
        mHeader.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                if(mHeader.getVisibility() == View.VISIBLE)
                    mClickCallback.onHandleClickForHeader();
            }
        });
    }

    public void setHeaderTouchListener(DayOnHeaderClickListener oClickCallback){
        mClickCallback = oClickCallback;
    ...
    public void setDayTitleToHeader(int date, int mon, int yr) {
        if(mHeaderConvertView != null) {
            DayOnEventListHeader holder = (DayOnEventListHeader) mHeaderConvertView.getTag();
            String sDayName =
                    labsCalendarUtils.getWeekForDate(date, mon, yr) +
                            ", " + labsCalendarUtils.getMonthName(mon) + " " + date + ", " + yr;
            holder.txtDayName.setText(sDayName);
            if(mon != mLastSetMonth || yr != mLastSetYear) {
                mLastSetMonth = mon;
                mLastSetYear = yr;
                //holder.imgView.setImageDrawable(ContextCompat.getDrawable(getContext(), labsCalendarUtils.getMonImg(mon)));
                String sMon = labsCalendarUtils.getMonthName(mon) + " " + yr;
                holder.txtMonthName.setText(sMon);
            }
        }
    }
